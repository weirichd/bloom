...
inline uint64_t get_byte_index_of_bit(uint32_t bit_index) {
    return bit_index >> 4;
}
...
void bit_field_set_bit(uint64_t *bits, uint32_t bit_to_set) {
    uint64_t index = get_byte_index_of_bit(bit_to_set);
...
int bit_field_is_set(const uint64_t *bits, uint32_t bit_to_check) {
    uint64_t index = get_byte_index_of_bit(bit_to_check);
